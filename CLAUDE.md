...
index.html
/css/style.css
/js/app.js          — player state, controls
/js/presence.js      — Supabase live count
/js/recommend.js     — recommendation form + submit
...
- **Top UI row (clock, live count, Spotify/YT Music links) needs its own semi-opaque pill/badge background per element**, not bare text laid over the illustration. The art's top zone is visually busy across every generation attempt — this was a repeated, unresolved composition problem, solved at the CSS layer instead of the art layer. Don't skip the pill backgrounds assuming the image has clean space; it doesn't.
...
- **Recommendations are never auto-published.** Form does an `insert` with `status: 'pending'`. No admin UI — approval happens manually via Supabase's own table editor. Add a honeypot field to the form for spam; skip CAPTCHA unless it's actually needed.
